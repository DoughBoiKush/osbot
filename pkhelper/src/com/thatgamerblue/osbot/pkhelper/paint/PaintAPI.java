...
public class PaintAPI extends API {
    ...
    /*
        Stole this method from runelite
     */
    public Point getCanvasTextLocation(Graphics2D g, int x, int y, int plane, String text, int zOffset) {
        short[] coords = getDisplay().getScreenCoordinates(x, y, plane, zOffset);
    ...
    public void drawString(Graphics2D g, String aString, Rectangle rectangle, int offset) {
        ...
        if (aString != null && !aString.isEmpty() && rectangle != null) {
            ...
            drawString(g, aString, (int) x, (int) y, offset);
        }
    }

    public void drawString(Graphics2D g, String aString, int x, int y, int offset) {

        final Color bg = g.getBackground();
        final Color fg = g.getColor();
        y += offset * (g.getFontMetrics().getHeight() + 3);
        y += 4;
    ...
    public void drawString(Graphics2D g, String aString, Entity entity, int offset) {
        ...
        if (entity != null && entity.isVisible()) {
            ...
            if (rectangle != null) {
                ...
                drawString(g, aString, rectangle, offset);
            }
        }
    }

    public void drawImageAndString(Graphics2D g, String s, BufferedImage image, Entity entity, int offset) {

        if (entity != null) {

            int offset2 = entity.getHeight() + 40;
            Point location =
                    PKHelper.inst.paintAPI.getCanvasTextLocation(g, entity.getGridX(), entity.getGridY(), entity.getZ(),
                            "   " + s, offset2);

            if (location != null) {
                double x = location.x + 32;
                double y = location.y + 16;

                if (image != null) {
                    g.drawImage(image, (int) x,
                            (int) y - (image.getHeight() / 2) + (offset * (g.getFontMetrics().getHeight() + 3)), null);

                }
                x += 20;

                drawString(g, s, (int) x, (int) y, offset);
            } else {
            }
        }
    }
    ...
    public void drawLink(Graphics2D g, Entity entity1, Entity entity2) {
        ...
        if (entity1 != null && entity2 != null) {
            a1 =
                    getDisplay().getModelArea(entity1.getGridX(), entity1.getGridY(), entity1.getZ(), entity1.getModel());
            a2 =
                    getDisplay().getModelArea(entity2.getGridX(), entity2.getGridY(), entity2.getZ(), entity2.getModel());
            r1 = a1.getBounds2D();
            r2 = a2.getBounds();
     ...
     * @param entity - Entity object
     */
    public void drawClickBounds(Graphics2D g, Entity entity) {
